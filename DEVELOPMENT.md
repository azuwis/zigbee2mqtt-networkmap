...
- `<v-style>` is a local component that uses `h('style', this.$slots.default())` to render CSS into a `<style>` tag. Placed outside `<ha-card>` to avoid being slotted into `<ha-card>`'s own Shadow DOM, which ensures styles apply correctly within `defineCustomElement`'s shadow root
...
- `transform(attr, config)`: converts raw HA sensor attributes into node/link arrays. Coordinator nodes get `name: ' '` (a space, not empty, because vue3-d3-network falls back to `"node <id>"` for empty strings). Links with dangling endpoints (nodes missing from the nodes array) are filtered out to prevent d3-force errors
